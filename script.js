...
// Service purchase functionality
function buyService(serviceName, price) {
    ...
    document.getElementById('paymentAmount').textContent = price + ' SOL';
    document.getElementById('paymentModal').classList.add('show');
    resetPaymentModal(); // Reset modal state when opening
}

// Modal functionality
function closeModal() {
    document.getElementById('paymentModal').classList.remove('show');
    resetPaymentModal(); // Reset modal state when closing
}

function copyAddress() {
    const address = '8NyFBUkCx35Biqde2GqiruJA91qwY1gD14bY8zQW9gA2';
...
function confirmPayment() {
    ...
    confirmBtn.classList.add('btn-secondary'); // Change style to indicate disabled state

    // Show loading spinner and update message
    statusArea.style.display = 'block';
    statusMessage.textContent = 'Processing transaction, please wait...';

    setTimeout(() => {
        statusMessage.textContent = 'Payment Confirmed!';
        // Hide spinner (assuming it's part of statusArea or can be hidden via CSS)
        statusArea.querySelector('.loading-spinner').style.display = 'none';

        // Optionally, change button text or enable for a "View Transaction" action
        confirmBtn.textContent = 'Confirmed!';
        confirmBtn.classList.remove('btn-secondary');
        confirmBtn.classList.add('btn-success');
        // You might want to keep it disabled or change its action
        // confirmBtn.disabled = false; // Enable if you want to allow further action

        // Auto-close modal after a short delay to show confirmation message
        setTimeout(() => {
            closeModal();
        }, 2000); // Close modal 2 seconds after confirmation
    }, 30000); // 30000 milliseconds = 30 seconds for auto-confirmation
}
...
function resetPaymentModal() {
    ...
    confirmBtn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg> I\'ve Sent The SOL';
    confirmBtn.classList.remove('btn-secondary');
    confirmBtn.classList.add('btn-success');

    // Hide status area and clear message
    statusArea.style.display = 'none';
    statusMessage.textContent = '';
    statusArea.querySelector('.loading-spinner').style.display = 'block'; // Ensure spinner is visible for next use
}
...
document.getElementById('tokenAddress').addEventListener('input', function(e) {
    ...
    if (value.length > 0 && (value.length < 32 || value.length > 44)) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    } else if (pagePath.includes('promote-coins.html')) {
        initializePaymentModal(); 
        initializePromoteCoinsPage(); 
    } else if (pagePath.includes('create-liquidity.html')) {
        initializePaymentModal();
